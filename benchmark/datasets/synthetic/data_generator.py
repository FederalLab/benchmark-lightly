...
class SyntheticDataset:
    def __init__(self,
        ...
        self.Q = np.random.normal(loc=0.0,
                                  scale=1.0,
                                  size=(self.num_dim + 1, self.num_classes,
                                        self.side_info_dim))
    ...
    def get_task(self, num_samples):
        cluster_idx = np.random.choice(range(self.num_clusters),
                                       size=None,
                                       replace=True,
                                       p=self.prob_clusters)
        new_task = self._generate_task(self.means[cluster_idx], cluster_idx,
                                       num_samples)
        return new_task
    ...
    def _generate_x(self, num_samples):
        ...
        samples[:, 1:] = np.random.multivariate_normal(mean=loc,
                                                       cov=self.Sigma,
                                                       size=num_samples)
    ...
    def _generate_y(self, x, cluster_mean):
        model_info = np.random.normal(loc=cluster_mean,
                                      scale=0.1,
                                      size=cluster_mean.shape)
        w = np.matmul(self.Q, model_info)

        num_samples = x.shape[0]
        prob = softmax(np.matmul(x, w) + np.random.normal(
            loc=0., scale=0.1, size=(num_samples, self.num_classes)),
                       axis=1)
