# @Author            : FederalLab
# @Date              : 2021-09-26 00:34:10
# @Last Modified by  : Chen Dengsheng
# @Last Modified time: 2021-09-26 00:34:10
...
if os.environ.get('LEAF_DATA_META_DIR') is not None:
    seed_fname = os.path.join(os.environ.get('LEAF_DATA_META_DIR'),
                              SEED_FILES['sampling'])  # type: ignore
...
for f in files:
    ...
    if (args.iid):
        ...
        while (ctot_num_samples < num_new_samples):
            ...
            if 'hierarchies' in data:
                hierarchies.append(hier)  # type: ignore
        ...
        if 'hierarchies' in data:
            users = [u for u, h in users_and_hiers][:user_i]  # type: ignore
        else:
            users = users[:user_i]
